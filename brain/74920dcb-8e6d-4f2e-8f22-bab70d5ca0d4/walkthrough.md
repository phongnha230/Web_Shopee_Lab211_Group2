# 📝 Tài liệu thay đổi code - Phiên 20/12/2025

## Tổng quan
Tài liệu này ghi lại tất cả các thay đổi code trong phiên làm việc này để dễ dàng tìm và sửa sau này.

---

## 1. Fix React Key Prop Warning

**File:** [AdminDashboardPage.jsx](file:///c:/Users/HPPAVILION/Documents/Cusor/Cafe_app/my-app/Frontend/src/pages/Admin/AdminDashboardPage.jsx#L1417-L1421)

**Vấn đề:** React warning "Each child in a list should have a unique 'key' prop"

**Thay đổi:**
```diff
- <div className="order-item" key={idx}>
+ <div className="order-item" key={`${it.productId}-${idx}`}>
```

---

## 2. Fix News Excerpt không hiển thị

**File:** [newsStore.js](file:///c:/Users/HPPAVILION/Documents/Cusor/Cafe_app/my-app/Frontend/src/stores/newsStore.js#L25)

**Vấn đề:** Mô tả tin tức không hiển thị trên trang chủ

**Thay đổi:**
```diff
- excerpt: news.excerpt || news.description || '',
+ excerpt: news.excerpt || news.content || news.description || '',
```

---

## 3. Fix News Image không lưu được (Backend)

**File:** [newController.js](file:///c:/Users/HPPAVILION/Documents/Cusor/Cafe_app/my-app/backend/controllers/newController.js#L80-L109)

**Vấn đề:** Backend chỉ nhận file upload, không nhận base64/URL

**Thay đổi:**
```diff
- const { title, content, status = 'draft' } = req.body;
+ const { title, content, status = 'draft', image_url } = req.body;

+ // Handle image: prioritize body image_url (base64/URL), then file upload
+ let finalImageUrl = null;
+ if (image_url) {
+   finalImageUrl = image_url;
+ } else if (req.file) {
+   finalImageUrl = `/uploads/${req.file.filename}`;
+ }

  const news = await News.create({
    title,
    content,
    status,
    created_by: userId,
-   image_url: req.file ? `/uploads/${req.file.filename}` : null
+   image_url: finalImageUrl
  });
```

---

## 4. Fix News Database Column quá nhỏ

**File:** [news.js](file:///c:/Users/HPPAVILION/Documents/Cusor/Cafe_app/my-app/backend/models/news.js#L8)

**Vấn đề:** Column `image_url` chỉ 255 ký tự, không đủ cho base64

**Thay đổi Model:**
```diff
- image_url: { type: DataTypes.STRING(255), allowNull: true },
+ image_url: { type: DataTypes.TEXT, allowNull: true },
```

**SQL Migration chạy:**
```sql
ALTER TABLE news MODIFY COLUMN image_url LONGTEXT;
```

---

## 5. Thêm section "Đánh giá của bạn"

**File:** [CustomerOrdersPage.jsx](file:///c:/Users/HPPAVILION/Documents/Cusor/Cafe_app/my-app/Frontend/src/pages/Order/CustomerOrdersPage.jsx#L160-L230)

**Vấn đề:** Không có nơi hiển thị các sản phẩm đã đánh giá

**Thêm mới:** Section hiển thị:
- Danh sách sản phẩm đã đánh giá
- Badge "✓ Đã đánh giá"
- Số sao đánh giá
- Nội dung comment
- Nút "Chỉnh sửa"

---

## 6. Lọc sản phẩm đã đánh giá khỏi "Đánh giá món đã mua"

**File:** [CustomerOrdersPage.jsx](file:///c:/Users/HPPAVILION/Documents/Cusor/Cafe_app/my-app/Frontend/src/pages/Order/CustomerOrdersPage.jsx#L107-L158)

**Vấn đề:** Sản phẩm đã đánh giá vẫn hiện trong danh sách cần đánh giá

**Thay đổi:** Thêm filter:
```javascript
.filter((it) => {
  const existingReview = getReview(it.productId, customerName || 'ẩn danh');
  return !existingReview; // Only keep items that haven't been reviewed
});
```

---

## 7. Fix Payment Method hiển thị sai

### 7.1 Backend - Include Payment

**File:** [orderController.js](file:///c:/Users/HPPAVILION/Documents/Cusor/Cafe_app/my-app/backend/controllers/orderController.js#L1-L5)

**Thêm import:**
```diff
+ const Payment = require('../models/payment');
```

**File:** [orderController.js](file:///c:/Users/HPPAVILION/Documents/Cusor/Cafe_app/my-app/backend/controllers/orderController.js#L89-L98)

**Thêm include Payment:**
```diff
  include: [
    { model: OrderItem, as: 'items' },
+   { model: Payment, as: 'payments' }
  ]
```

### 7.2 Frontend - Hiển thị đúng method

**File:** [CustomerOrdersPage.jsx](file:///c:/Users/HPPAVILION/Documents/Cusor/Cafe_app/my-app/Frontend/src/pages/Order/CustomerOrdersPage.jsx#L91-L100)

**Thay đổi:**
```diff
- <td>{o.paymentMethod === 'vnpay' ? 'VNPay' : 'Tiền mặt'}</td>
+ <td>
+   {(() => {
+     const payment = o.payments?.[0];
+     const method = payment?.method || o.paymentMethod || 'cash';
+     if (method === 'vnpay') return 'VNPay';
+     if (method === 'momo') return 'MoMo';
+     return 'Tiền mặt';
+   })()}
+ </td>
```

---

## Danh sách file đã sửa

| File | Dòng | Mô tả |
|------|------|-------|
| `Frontend/src/pages/Admin/AdminDashboardPage.jsx` | 1421 | Fix React key prop |
| `Frontend/src/stores/newsStore.js` | 25 | Fix news excerpt mapping |
| `Frontend/src/pages/Home/HomePage.jsx` | 134-141 | Fix news excerpt display |
| `Frontend/src/pages/Order/CustomerOrdersPage.jsx` | 91-100, 107-230 | Payment method + Reviews UI |
| `backend/controllers/newController.js` | 80-109 | Accept base64 images |
| `backend/controllers/orderController.js` | 1-5, 89-98 | Include Payment data |
| `backend/models/news.js` | 8 | Change image_url to TEXT |

---

## Lưu ý

> [!IMPORTANT]
> Sau khi sửa backend, cần **restart server** để áp dụng thay đổi.

> [!WARNING]
> Database migration `ALTER TABLE news MODIFY COLUMN image_url LONGTEXT` cần được chạy nếu chưa có.
